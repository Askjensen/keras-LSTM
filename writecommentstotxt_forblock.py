...
def danskebogstavertiltegn(instring):
    ...
    return instring.replace(u'å', '?').replace(u'æ', '!').replace(u'ø', '&').replace(u'ü', 'u').replace(u'ö', 'o').replace(u'ä', 'a').replace(u'Å','?').replace(u'Æ','!').replace(u'Ø','&')
...
def splitInComments(i,comments):
    return list(itertools.chain(*[split_sentences(com) for com in comments[comments[i]]]))
# In[3]:

readInData = False
askwritecommentstsfile = False
readInTexts = True
...
for party in comments:
    ...
    if(askwritecommentstsfile):
        ...
        #for i in range(100): #len(comments)):
            #comment_list = comment_list + split_sentences(comments[i])
            #comment_list = comment_list + comments[i]
    ...
    comment_list = comments[party]

    texts = []
    if(readInTexts):
        #texts = [document for document in comment_list]  #
        texts = [word for word in [clean_post(document.encode('utf-8')).split() for document in comment_list]] #
        #
        #
